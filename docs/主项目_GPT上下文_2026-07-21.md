...
### 5.0 当前活动任务：C52 Mic 内部堆碎片与 VAD 门控修复

- 预修复串口证据表明 C52 `mic_adc_test` 请求 `12288 B` internal task stack 时，内部总空闲约为
  `18-24 KiB`，但最大连续块只有 `11264 B`，所以 `xTaskCreateWithCaps` 返回
  `ESP_ERR_NO_MEM`；随后 Wi-Fi 仍达到 `LINK_READY`，不能把它归因于 Wi-Fi 故障。
- 该旧路径约每 `500-700 ms` 重试，并反复创建 event group、分配 `16000 B` PSRAM pre-roll 后才失败。
  当前要求改为预分配的安全任务栈、完整幂等回滚、单一启动事务和有界指数退避；禁止以关闭 Wi-Fi、BLE、
  LCD、scheduler 或 gateway link 规避问题。
- C5 只保留 PCM 采集、轻量 VAD 和有限 pre/post-roll；静音时不可持续向 S3 传 PCM。WakeNet/唤醒词与
  重型后续处理仍由 S3 负责。完整的持续任务记录见
  `docs/development_logs/task_espc52_mic_internal_heap_fragmentation_repair_2026-07-21.md`。
- 当前源码已将 `mic_adc_test` 改为 `12288 B` internal-static stack/TCB，避免动态申请连续内部堆；
  `voice_chain` 实现 single-flight 与 `1/2/4/8/15/30 s` 退避，传输层记录 PCM stream/drop/pressure
  并清理 partial init。ESP-IDF 完整 build 仍待本任务独立记录，所有硬件与 cache-off/OTA 压力结论仍待验证。
...
| LCD/LVGL | ESPC52 已在保留的 LCD service/LVGL 生命周期内完成 C51 风格 Dashboard、Boot readiness、环境/网络字段、Pixel Cat、Voice Overlay 和 queue-only Cat wake；C51/C52 独立构建均通过 | 仍需目标板验证颜色、方向、触摸、动画、语音/雷达并发及既有 deinit/cache-off 风险 |
...
- `docs/主项目_ESPC52_LCD_C51视觉交互同步执行记录_2026-07-21.md`：ESPC52 在既有 LCD/LVGL 生命周期内完成 ESPLCD C51 视觉交互同步的多 Agent 执行记录，含修改范围、审查、两端构建结果及硬件待验项。
