...
#define ESP111_PROTOCOL_SERVER_ROUTE_COMMANDS_PENDING "/api/commands/pending"
                                                   ...
                                                   size_t out_capacity)
{
    if (frame == NULL || out == NULL || frame->payload_length > ESP111_PROTOCOL_AUDIO_STREAM_MAX_PAYLOAD ||
        ...
        (frame->payload_length > 0U && payload == NULL)) {
        return 0U;
    }
    out[0] = ESP111_PROTOCOL_AUDIO_STREAM_VERSION;
    out[1] = frame->type;
    out[2] = frame->source_id;
    out[3] = frame->flags;
    ...
    out[18] = frame->bits_per_sample;
    out[19] = frame->channels;
    ...
    if (frame->payload_length > 0U) {
        memcpy(out + ESP111_PROTOCOL_AUDIO_STREAM_HEADER_BYTES, payload, frame->payload_length);
    }
    uint16_t crc = esp111_protocol_audio_crc16(out, 24U);
    if (frame->payload_length > 0U) {
        crc ^= esp111_protocol_audio_crc16(out + ESP111_PROTOCOL_AUDIO_STREAM_HEADER_BYTES,
                                           frame->payload_length);
    }
    esp111_protocol_audio_write_u16(out + 24U, crc);
    out[26] = 0U;
    out[27] = 0U;
    return ESP111_PROTOCOL_AUDIO_STREAM_HEADER_BYTES + frame->payload_length;
}

static inline bool esp111_protocol_audio_decode(const uint8_t *packet,
                                                size_t packet_length,
                                                esp111_protocol_audio_frame_t *out,
                                                const uint8_t **payload)
{
    if (packet == NULL || out == NULL || packet_length < ESP111_PROTOCOL_AUDIO_STREAM_HEADER_BYTES ||
        packet[0] != ESP111_PROTOCOL_AUDIO_STREAM_VERSION) {
        return false;
    }
    const uint16_t payload_length = esp111_protocol_audio_read_u16(packet + 22U);
    if (payload_length > ESP111_PROTOCOL_AUDIO_STREAM_MAX_PAYLOAD ||
        packet_length != ESP111_PROTOCOL_AUDIO_STREAM_HEADER_BYTES + payload_length) {
        return false;
    }
    ...
    out->crc16 = esp111_protocol_audio_read_u16(packet + 24U);
    uint16_t crc = esp111_protocol_audio_crc16(packet, 24U);
    if (payload_length > 0U) {
        crc ^= esp111_protocol_audio_crc16(packet + ESP111_PROTOCOL_AUDIO_STREAM_HEADER_BYTES,
                                           payload_length);
    }
    if (crc != out->crc16) {
        return false;
