...
static esp_err_t prepare_device_histories(alarm_device_runtime_t *device)
{
    esp_err_t ret = alarm_history_allocate(&device->temperature_history,
                                            ALARM_ENGINE_TEMPERATURE_HISTORY_CAPACITY);
    if (ret != ESP_OK) return ret;
    ret = alarm_history_allocate(&device->humidity_history,
                                 ALARM_ENGINE_HUMIDITY_HISTORY_CAPACITY);
    if (ret != ESP_OK) return ret;
    return alarm_history_allocate(&device->air_quality_history,
...
{
    const alarm_history_t temperature = device->temperature_history;
    const alarm_history_t humidity = device->humidity_history;
    const alarm_history_t air_quality = device->air_quality_history;
...
esp_err_t alarm_engine_init(const alarm_engine_options_t *options) {
    ...
    for (size_t i = 0U; i < ALARM_ENGINE_MAX_DEVICES; ++i) {
        const esp_err_t ret = prepare_device_histories(&g_alarm_engine.devices[i]);
        if (ret != ESP_OK) {
            for (size_t release = 0U; release <= i; ++release) release_device_histories(&g_alarm_engine.devices[release]);
            return ret;
        }
    }
    portENTER_CRITICAL(&s_lock);
