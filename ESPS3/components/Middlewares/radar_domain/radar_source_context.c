...
{
    const RadarSourceContext *context = radar_source_context_get(source);
    if (context == NULL || out == NULL || !state_lock()) return false;
                                       ...
                                       radar_source_context_log_view_t *out)
{
    ...
    copy_text(out->source_name, sizeof(out->source_name), context->source_name);
    copy_text(out->device_id, sizeof(out->device_id), context->device_id);
    copy_text(out->room_id, sizeof(out->room_id), context->room_id);
    out->sequence = context->sequence;
    out->frame_sequence = context->frame_sequence;
    out->snapshot = context->snapshot;
    state_unlock();
    return true;
}
