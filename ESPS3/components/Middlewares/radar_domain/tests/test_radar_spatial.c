...
static void test_spatial_semantics(void)
{
    ...
    empty.frame_seq = 16U;
    empty.received_at_ms = 13000U;
    radar_spatial_state_on_frame(&state, &empty, true, 13000U);
    radar_spatial_state_poll(&state, RADAR_UART_RECOVERY_VALID, 13401U);
    radar_spatial_state_get_snapshot(&state, &snapshot);
...
static void test_single_person_five_minute_stability(void)
{
    ...
    assert(snapshot.source_person_count == 1U);
...
static void test_person_continuity_snapshot_counts(void)
{
    ...
    for (uint32_t sequence = 1U; sequence <= 4U; ++sequence) {
        ...
        assert(snapshot.source_person_count <= 1U);
        if (sequence == 2U) {
            assert(snapshot.source_person_count == 0U);
