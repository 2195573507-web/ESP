...
static void refresh_counts(radar_person_continuity_t *continuity)
{
    ...
    continuity->business_person_count = 0U;
    ...
    for (size_t i = 0U; i < RADAR_PERSON_CONTINUITY_MAX_PERSONS; ++i) {
        ...
        if (person->state == RADAR_PERSON_VISIBLE) {
            ...
            ++continuity->business_person_count;
                      ...
                      const char *reason)
{
    ...
    ESP_LOGI(TAG,
             "%s source=%s person_id=%lu old_track_id=%lu new_track_id=%lu distance_mm=%lu age_ms=%lu zone=%u visible_person_count=%u retained_person_count=%u business_person_count=%u count_state=%s reason=%s",
             event,
             source_name(continuity->source_id),
             ...
             (unsigned int)continuity->business_person_count,
                                    ...
                                    uint64_t now_ms)
{
    ...
    const uint8_t previous_business = continuity->business_person_count;
    ...
    if (previous_visible != continuity->visible_person_count ||
        ...
        previous_business != continuity->business_person_count ||
                                        ...
                                        radar_person_count_state_t *count_state)
{
    ...
    if (business_person_count != NULL) {
        *business_person_count = continuity != NULL ? continuity->business_person_count : 0U;
    }
    if (count_state != NULL) {
