...
{
    (void)arg;
    app_stack_monitor_report(TAG, "radar_log", RADAR_CONFIG_LOG_TASK_STACK, "entry");
    ...
    while (!s_stop_requested) {
        ...
        if (stack_pending) {
            radar_source_context_log_view_t local = {0};
            const bool have_local = radar_source_context_get_log_view(RADAR_SOURCE_S3_LOCAL,
                                                                        &local);
            ESP_LOGI(TAG,
                     ...
                     have_local ? local.source_name : "S3_LOCAL",
                     have_local ? local.device_id : "unknown",
                     have_local ? local.room_id : "unknown",
                     (unsigned long)(have_local ? local.sequence : 0U));
        }
    ...
    portENTER_CRITICAL(&s_lock);
    memset(s_pending_contexts, 0, sizeof(s_pending_contexts));
    s_stack_pending = false;
    s_task = NULL;
    portEXIT_CRITICAL(&s_lock);
    if (s_exit != NULL) {
                               ...
                               const radar_rate_manager_t *rate_manager)
{
    ...
    const radar_source_id_t source = context->source_id;
    if (!radar_source_context_get_log_view(source, &view)) return;
#if !CONFIG_S3_RADAR_PER_SOURCE_LOG_SCHEDULER
    ...
    s_pending_contexts[source].snapshot = view.snapshot;
