...
static void delete_adapter_mutex(SemaphoreHandle_t lock)
{
#ifdef ESP_PLATFORM
    if (lock != NULL) {
...
esp_err_t environment_alarm_adapter_init(void)
{
    ...
    const bool lock_created = lock == NULL;
    if (lock_created) {
        lock = xSemaphoreCreateMutex();
        if (lock == NULL) return ESP_ERR_NO_MEM;
    }

    alarm_engine_diagnostics_t diagnostics = {0};
    const bool engine_was_initialized = alarm_engine_get_diagnostics(&diagnostics) == ESP_OK;
    esp_err_t ret = alarm_engine_init(NULL);
    if (ret != ESP_OK) {
        if (lock_created) delete_adapter_mutex(lock);
        return ret;
    }
    ret = environment_alarm_reporter_init();
    if (ret != ESP_OK) {
        if (!engine_was_initialized) alarm_engine_deinit();
        if (lock_created) delete_adapter_mutex(lock);
        return ret;
    }
