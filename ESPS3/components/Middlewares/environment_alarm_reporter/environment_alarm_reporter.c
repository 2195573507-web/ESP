...
#ifdef ESP_PLATFORM
static StaticQueue_t s_completion_queue_control;
static uint8_t *s_completion_queue_storage;
#endif
...
static environment_alarm_observer_t s_observer;
static void *s_observer_context;

static QueueHandle_t create_completion_queue(void)
{
#ifdef ESP_PLATFORM
    const size_t bytes = ENV_ALARM_REPORTER_COMPLETION_DEPTH *
                         sizeof(environment_alarm_completion_t);
    s_completion_queue_storage = ENV_ALARM_STORAGE_ALLOC(1U, bytes);
    if (s_completion_queue_storage == NULL) {
        return NULL;
    }
    QueueHandle_t queue = xQueueCreateStatic(ENV_ALARM_REPORTER_COMPLETION_DEPTH,
                                              sizeof(environment_alarm_completion_t),
                                              s_completion_queue_storage,
                                              &s_completion_queue_control);
    if (queue == NULL) {
        ENV_ALARM_STORAGE_FREE(s_completion_queue_storage);
        s_completion_queue_storage = NULL;
    }
    return queue;
#else
    return xQueueCreate(ENV_ALARM_REPORTER_COMPLETION_DEPTH,
                        sizeof(environment_alarm_completion_t));
#endif
}

static void delete_completion_queue(QueueHandle_t queue)
{
#ifdef ESP_PLATFORM
    if (queue != NULL) {
        vQueueDelete(queue);
    ...
    ENV_ALARM_STORAGE_FREE(s_completion_queue_storage);
    s_completion_queue_storage = NULL;
    memset(&s_completion_queue_control, 0, sizeof(s_completion_queue_control));
#else
    (void)queue;
#endif
}

static void delete_reporter_mutex(SemaphoreHandle_t lock)
{
#ifdef ESP_PLATFORM
    if (lock != NULL) {
...
{
    if (event == NULL || s_storage == NULL || device_name(event->device_id)[0] == '\0') {
        return false;
    }
...
static void submit_head_if_due(void)
{
    ...
    char *json_body = ENV_ALARM_STORAGE_ALLOC(json_length + 1U, sizeof(char));
    if (json_body == NULL) {
    ...
    const esp_err_t ret = network_worker_submit_environment_alarm_json(json_body,
                                                                         ...
                                                                         "environment_alarm");
    if (ret != ESP_OK) {
        ENV_ALARM_STORAGE_FREE(json_body);
...
esp_err_t environment_alarm_reporter_init(void)
{
    ...
    environment_alarm_reporter_storage_t *storage =
        ENV_ALARM_STORAGE_ALLOC(1U, sizeof(*storage));
    if (storage == NULL) {
        return ESP_ERR_NO_MEM;
    }
    SemaphoreHandle_t lock = xSemaphoreCreateMutex();
    if (lock == NULL) {
        ENV_ALARM_STORAGE_FREE(storage);
        return ESP_ERR_NO_MEM;
    }
    QueueHandle_t completion_queue = create_completion_queue();
    if (completion_queue == NULL) {
        delete_reporter_mutex(lock);
        ENV_ALARM_STORAGE_FREE(storage);
        return ESP_ERR_NO_MEM;
    }
    ...
    if (created != pdPASS) {
        ...
        s_lock = NULL;
        s_storage = NULL;
        delete_completion_queue(completion_queue);
        delete_reporter_mutex(lock);
        ENV_ALARM_STORAGE_FREE(storage);
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t environment_alarm_reporter_drain_engine(void)
{
    ...
    uint8_t observer_indices[ALARM_ENGINE_MAX_EVENTS_PER_UPDATE] = {0};
    size_t observer_count = 0U;
    environment_alarm_observer_t observer = NULL;
    ...
    for (size_t index = 0U; index < event_count; ++index) {
        ...
        observer_indices[observer_count++] = (uint8_t)index;
        ack_through = event->event_seq;
        ESP_LOGI(TAG,
    ...
    s_stats.report_queue_depth = s_count;
    observer = s_observer;
    observer_context = s_observer_context;
    xSemaphoreGive(s_lock);

    if (observer != NULL) {
        for (size_t index = 0U; index < observer_count; ++index) {
            observer(&s_drain_events[observer_indices[index]], observer_context);
        }
    }
