...
/**
 * @file s3_scheduler.h
 * @brief ESPS3 统一运行时事件队列和自适应调度器。
 *
 * HTTP、UDP stream、CSI、command、network state 和周期任务都先进入本模块。
 * scheduler 是 S3 业务事件的统一消费入口，也统一拥有 CSI tick、snapshot upload、
 * command pull、smart-home poll 等周期 cadence。WiFi callback 和 HTTP handler
 * 只入队，不直接做重业务。
 */
...
/** S3 runtime bus ingress；body 是 C5 原始本地 payload，unified 是用于日志/快照的扁平摘要。 */
typedef struct {
...
/** @brief 本地 C5<->S3 socket 是否允许发送；STA connected/IP ready/link stable 均为 true。 */
