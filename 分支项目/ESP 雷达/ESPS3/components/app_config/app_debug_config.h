 ...
 *
 * 调用方法：
 * 1. 需要排查 Mic / VAD / server voice turn 时，只修改本文件里的 APP_DEBUG_* 宏。
...
/* Server voice turn 关键日志：默认只保留主流程一句话日志，细节由调试开关打开。 */
...
/* VAD/server voice 默认参数：集中在这里方便现场调试，不改变 WiFi、ADC 或 PCM 转换链路。 */
#define APP_VOICE_AUDIO_PACKET_MS                    100  // server voice 音频包时长，当前 100 ms。
...
#define APP_VOICE_VAD_START_COOLDOWN_MS             1200 // server voice done 后 VAD 起始冷却，防止刚恢复就再次触发。
...
#define APP_VOICE_PRE_SPEECH_PACKETS                5    // 句首预缓存 5 个 100 ms server voice PCM 包。
...
/* Bridge 调试：活动固件只通过 server_voice_client 与 ESP-server 通信。 */
