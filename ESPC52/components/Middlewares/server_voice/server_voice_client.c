...
#define SERVER_VOICE_COMMAND_ID_MAX_LEN 64U
...
typedef struct {
    ...
    char command_id[SERVER_VOICE_COMMAND_ID_MAX_LEN];
    bool command_metadata_set;
...
static StackType_t *s_voice_response_task_stack;
...
static void server_voice_clear_command_capture_metadata(void)
{
    s_voice.command_metadata_set = false;
    s_voice.command_stream_id = 0U;
    s_voice.command_generation = 0U;
    s_voice.command_id[0] = '\0';
}
                                                  ...
                                                  void *user_ctx)
{
    ...
    if (!ctx->stream_open) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "PLAYBACK_BEGIN result=failed reason=playback_start_callback ret=%s",
                     esp_err_to_name(ret));
            return ret;
        }
        ...
        if (ret != ESP_OK) {
            ...
            ESP_LOGE(TAG, "PLAYBACK_BEGIN result=failed reason=audio_stream_open ret=%s",
                     esp_err_to_name(ret));
            return ret;
        }
        (void)c5_resource_manager_note_phase(lease, "after_i2s_init");
        ctx->stream_open = true;
        ESP_LOGI(TAG, "PLAYBACK_BEGIN result=ok reason=first_server_pcm_chunk generation=%lu",
                 (unsigned long)ctx->generation);
    }
...
static void server_voice_response_process(void)
{
    ...
    const char *failure_message = "server voice turn failed";
    ...
    if (ret == ESP_OK) {
        ...
        if (status == 204) {
            ...
            ESP_LOGE(TAG, "VOICE_RESPONSE_FAILED reason=server_unavailable status=503");
            failure_message = "server_unavailable";
            ret = ESP_FAIL;
            ...
            if (playback_ctx == NULL) {
                ...
                if (playback_ctx->stream_open) {
                    ...
                    ESP_LOG_LEVEL_LOCAL(finish_ret == ESP_OK ? ESP_LOG_INFO : ESP_LOG_ERROR,
                                        TAG,
                                        "PLAYBACK_END result=%s reason=%s ret=%s generation=%lu",
                                        finish_ret == ESP_OK ? "ok" : "failed",
                                        server_voice_abort_requested() ? "voice_turn_aborted" :
                                                                           "server_response_complete",
                                        esp_err_to_name(finish_ret),
                                        (unsigned long)playback_ctx->generation);
    ...
    server_voice_clear_command_capture_metadata();
    ...
    if (ret == ESP_OK && !aborted) {
        ...
        ESP_LOGE(TAG,
                 ...
                 "transport_error");
        server_voice_emit_error(completed_lease_generation, (int)ret, failure_message);
    }
...
void server_voice_client_set_network_available(bool available, const char *reason)
{
    ...
    if (s_voice.network_available != available) {
        ESP_LOGI(TAG,
                 "VOICE_NETWORK_STATE source=server_voice_client available=%d state=%s reason=%s",
                 available ? 1 : 0,
                 server_voice_state_name(s_voice.state),
...
{
    if (!s_voice.initialized || !s_voice.network_available) {
        return ESP_ERR_INVALID_STATE;
    }
    if (s_voice.state != SERVER_VOICE_STATE_IDLE || s_voice.response_active) {
        return ESP_ERR_INVALID_STATE;
    }

    ESP_LOGI(TAG, "VOICE_RECONNECT_START target=server_voice_turn");
    const esp_err_t ret = server_voice_client_prepare_async();
    ESP_LOGI(TAG,
             "VOICE_RECONNECT_DONE target=server_voice_turn ret=%s",
             esp_err_to_name(ret));
    return ret;
}
...
esp_err_t server_voice_client_set_command_capture_metadata(const char *command_id,
                                                           ...
                                                           uint32_t command_generation)
{
    if (!s_voice.initialized || command_id == NULL || command_id[0] == '\0' ||
        command_stream_id == 0U || command_generation == 0U ||
        s_voice.state != SERVER_VOICE_STATE_IDLE) {
        return ESP_ERR_INVALID_STATE;
    }
    s_voice.command_stream_id = command_stream_id;
    s_voice.command_generation = command_generation;
    strlcpy(s_voice.command_id, command_id, sizeof(s_voice.command_id));
    s_voice.command_metadata_set = true;
    return ESP_OK;
}
...
esp_err_t server_voice_client_start_turn(void)
{
    ...
    if (s_voice.command_metadata_set &&
        (s_voice.command_id[0] == '\0' || s_voice.command_stream_id == 0U ||
         s_voice.command_generation == 0U)) {
        ESP_LOGW(TAG, "command voice start rejected: incomplete command identity");
        server_voice_clear_command_capture_metadata();
        return ESP_ERR_INVALID_STATE;
    }
    ...
    ESP_LOGI(TAG,
             "local gateway voice turn begin gateway_id=%s device_id=%s command_stream_id=%lu command_generation=%lu",
             server_comm_get_gateway_id(),
             server_comm_get_device_id(),
             (unsigned long)s_voice.command_stream_id,
             (unsigned long)s_voice.command_generation);
    return ESP_OK;
}
...
esp_err_t server_voice_client_finish_turn(void)
{
    ...
    server_comm_header_t headers[DEVICE_PROTOCOL_MAX_HEADERS + 5U];
    char command_stream_id[12];
    char command_generation[12];
    ...
    if (s_voice.command_metadata_set) {
        (void)snprintf(command_stream_id, sizeof(command_stream_id), "%lu",
                       (unsigned long)s_voice.command_stream_id);
        (void)snprintf(command_generation, sizeof(command_generation), "%lu",
                       (unsigned long)s_voice.command_generation);
        headers[header_count++] = (server_comm_header_t) {
            .key = "X-Command-Stream-Id", .value = command_stream_id,
        };
        headers[header_count++] = (server_comm_header_t) {
            .key = "X-Command-Generation", .value = command_generation,
        };
        headers[header_count++] = (server_comm_header_t) {
            .key = "X-Command-Id", .value = s_voice.command_id,
        };
        headers[header_count++] = (server_comm_header_t) {
            .key = "X-Command-State", .value = "CAPTURE_END",
        };
    }
