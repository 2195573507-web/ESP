...
static void process_task(void *arg)
{
    ...
    while (true) {
        ...
        if (status.state != s_last_state) {
            ...
            ESP_LOGI(TAG,
                     "RADAR_SOURCE_STATE event=ble_state source_id=%u source=C52 device_id=%s room=%s sequence=0 local_id=%u state=%s link_online=%u",
...
esp_err_t radar_ble_runtime_start(void)
{
    ...
    if (transport_ret != 0) {
        ESP_LOGW(TAG,
                 "RADAR_SOURCE_STATE event=ble_unavailable source_id=%u source=C52 device_id=%s room=%s sequence=0 local_id=%u reason=BLOCKED_BY_RADAR_GATT_UUID ret=%d",
                 (unsigned int)RADAR_BLE_BINDING_LOCAL_ID,
                 RADAR_BLE_BINDING_DEVICE_ID,
                 RADAR_BLE_BINDING_ROOM_ID,
                 (unsigned int)RADAR_BLE_BINDING_LOCAL_ID,
