                                       ...
                                       int64_t release_duration_ms)
{
    ESP_LOGI(TAG,
             ...
             "psram_largest=%u standby_voice_session_bytes=%u",
...
static void c5_resource_rollback_quiesce(const char *reason, bool workers_may_be_paused)
{
    /* c5_runtime_workers_quiesce() sets its pause gate before it can time out. */
    if (workers_may_be_paused) {
        c5_runtime_workers_resume();
    }
    (void)app_runtime_resume_non_voice(reason);
}

static bool c5_resource_audio_phase_transition_allowed(c5_audio_phase_t from,
                                                        c5_audio_phase_t to)
{
    if (from == to) {
        return true;
    ...
    case C5_AUDIO_PHASE_MIC_DMA_RELEASED:
        return to == C5_AUDIO_PHASE_SPEAKER_TX_OWNED;
...
static void c5_resource_release_attempt_complete(void)
{
    portENTER_CRITICAL(&s_resource_lock);
    s_resource.release_in_progress = false;
    portEXIT_CRITICAL(&s_resource_lock);
}
...
esp_err_t c5_resource_manager_begin_voice(const char *owner, c5_voice_lease_t *out_lease)
{
    ...
    bool workers_may_be_paused = false;
...
fail:
    c5_resource_set_state(C5_RESOURCE_STATE_ERROR,
                          ...
                          0,
                          0);
    c5_resource_rollback_quiesce("voice_lease_rollback", workers_may_be_paused);
    portENTER_CRITICAL(&s_resource_lock);
                                              ...
                                              const char *reason)
{
    ...
    valid = s_resource.lease_valid && lease.generation != 0U &&
            ...
            (state == C5_RESOURCE_STATE_VOICE_EXCLUSIVE ||
             state == C5_RESOURCE_STATE_RELEASING) &&
            !s_resource.release_in_progress &&
            c5_resource_audio_phase_transition_allowed(old_phase, phase);
    if (valid) {
...
esp_err_t c5_resource_manager_release_voice(c5_voice_lease_t lease, const char *reason)
{
    ...
    if (s_resource.release_in_progress) {
        strlcpy(owner, s_resource.owner, sizeof(owner));
        portEXIT_CRITICAL(&s_resource_lock);
        ESP_LOGW(TAG,
                 "release already in progress generation=%lu owner=%s",
                 (unsigned long)lease.generation,
                 owner);
        return ESP_ERR_INVALID_STATE;
    }
    s_resource.release_in_progress = true;
    if (release_retry) {
    ...
    if (first_ret != ESP_OK) {
        ...
        c5_resource_release_attempt_complete();
        return first_ret;
    }
    ...
    portENTER_CRITICAL(&s_resource_lock);
    if (s_resource.lease_valid && lease.generation == s_resource.generation) {
        s_resource.audio_phase = C5_AUDIO_PHASE_SPEAKER_TX_RELEASED;
    }
    portEXIT_CRITICAL(&s_resource_lock);
    ESP_LOGI(TAG,
             "C5_AUDIO_PHASE generation=%lu phase=%s reason=release_audio_complete",
             (unsigned long)lease.generation,
             c5_resource_manager_audio_phase_name(C5_AUDIO_PHASE_SPEAKER_TX_RELEASED));
