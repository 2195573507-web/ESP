...
#define SERVER_COMM_VOICE_CONTROL_COMMAND_ID_BYTES 80U
...
static bool server_comm_voice_control_request_is_active_for_current_task(
    const char *endpoint,
...
static TaskHandle_t s_voice_control_request_task;
static uint32_t s_voice_control_generation;
static char s_voice_control_command_id[SERVER_COMM_VOICE_CONTROL_COMMAND_ID_BYTES];
...
{
    portENTER_CRITICAL(&s_server_comm_lock);
    if (active && command_id != NULL && command_id[0] != '\0' && generation != 0U) {
        s_voice_control_request_task = xTaskGetCurrentTaskHandle();
        s_voice_control_generation = generation;
        strlcpy(s_voice_control_command_id,
                command_id,
                sizeof(s_voice_control_command_id));
    } else {
        s_voice_control_request_task = NULL;
        s_voice_control_generation = 0U;
        s_voice_control_command_id[0] = '\0';
    }
    portEXIT_CRITICAL(&s_server_comm_lock);
}
    ...
    uint32_t *out_generation)
{
    ...
    char command_id[SERVER_COMM_VOICE_CONTROL_COMMAND_ID_BYTES] = {0};

    portENTER_CRITICAL(&s_server_comm_lock);
    task = s_voice_control_request_task;
    generation = s_voice_control_generation;
    strlcpy(command_id, s_voice_control_command_id, sizeof(command_id));
    portEXIT_CRITICAL(&s_server_comm_lock);

    if (out_generation != NULL) {
        *out_generation = 0U;
    }
    if (task == NULL || task != xTaskGetCurrentTaskHandle() || generation == 0U ||
        command_id[0] == '\0') {
        return false;
    }

    char expected[SERVER_COMM_VOICE_CONTROL_COMMAND_ID_BYTES +
                  sizeof(ESP111_PROTOCOL_ROUTE_COMMANDS_PREFIX) +
                  sizeof(ESP111_PROTOCOL_ROUTE_COMMAND_ACK_SUFFIX)] = {0};
    const int written = snprintf(expected,
                                 sizeof(expected),
                                 "%s%s%s",
                                 ESP111_PROTOCOL_ROUTE_COMMANDS_PREFIX,
                                 command_id,
                                 ESP111_PROTOCOL_ROUTE_COMMAND_ACK_SUFFIX);
    if (written <= 0 || written >= (int)sizeof(expected) ||
        !server_comm_path_matches(server_comm_endpoint_path(endpoint), expected)) {
        return false;
    }
                                                        ...
                                                        bool *out_admitted)
{
    ...
    *out_admitted = false;
    ...
    const bool reconnect_request = server_comm_request_is_reconnect_for_current_task();
    uint32_t voice_control_generation = 0U;
    const bool voice_control_request = server_comm_voice_control_request_is_active_for_current_task(
        endpoint, &voice_control_generation);
    if (voice_control_request) {
        ESP_LOGI(TAG,
                 "HTTP_VOICE_BUSY_ADMISSION endpoint=%s request_class=VOICE_CONTROL generation=%lu decision=allow reason=current_command_ack",
                 endpoint,
                 (unsigned long)voice_control_generation);
    }
    if (voice_request || wake_prompt_request || voice_turn_endpoint || voice_control_request ||
        reconnect_request) {
        return ESP_OK;
    }
...
static esp_err_t server_comm_check_ready(const char *label, const char *endpoint)
{
    ...
    bool voice_control_request =
        server_comm_voice_control_request_is_active_for_current_task(endpoint, NULL);

    if (!voice_request && !reconnect_request && !wake_prompt_request &&
        !voice_turn_endpoint && !voice_control_request && server_comm_non_voice_paused()) {
        server_comm_log_voice_busy_skip(label, endpoint);
        ESP_LOGI(TAG,
                 "HTTP_VOICE_BUSY_ADMISSION endpoint=%s request_class=NORMAL generation=0 decision=block reason=voice_busy",
    ...
    if (!reconnect_request) {
        if (voice_request || voice_turn_endpoint || voice_control_request) {
            /* Command capture is local-S3 work.  A cloud/server outage must
             * still reach voice_proxy so it can return server_unavailable. */
                                     ...
                                     server_comm_http_response_t *response)
{
    ...
    if (url == NULL) {
        ...
        gateway_link_record_http_result(ESP_ERR_NO_MEM,
                                        voice_request || voice_control_request,
                                        reconnect_request);
        server_comm_end_non_voice_admission(non_voice_admitted);
        return ESP_ERR_NO_MEM;
    }
    ...
    if (ret != ESP_OK) {
        ...
        heap_caps_free(url);
        gateway_link_record_http_result(ret, voice_request || voice_control_request, reconnect_request);
        server_comm_end_non_voice_admission(non_voice_admitted);
        return ret;
    }
    ...
    gateway_link_record_http_result(ESP_OK, voice_request || voice_control_request, reconnect_request);
    return ESP_OK;
}
