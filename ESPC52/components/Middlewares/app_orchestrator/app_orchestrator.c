...
static void c5_orchestrator_continue(void)
{
    ...
    if (radar_ble_ret != ESP_OK) {
        ESP_LOGW(TAG, "BOOT_STAGE stage=radar_ble state=degraded ret=%s", esp_err_to_name(radar_ble_ret));
    } else {
    ...
    if (MAIN_ENABLE_MIC_CHAIN && c5_scheduler_is_dispatcher_ready()) {
        ...
        if (voice_ret != ESP_OK) {
            ESP_LOGW(TAG, "BOOT_STAGE stage=local_voice state=degraded ret=%s", esp_err_to_name(voice_ret));
            c5_startup_resource_transition(C5_STARTUP_RESOURCE_VOICE_INIT_DENIED,
    ...
    if (gateway_ready) {
        c5_startup_stage_transition(C5_STARTUP_NETWORK_READY, "gateway_link_ready", ESP_OK);
    } else {
        const esp_err_t offline_ret = network_ret == ESP_OK ? ESP_ERR_TIMEOUT : network_ret;
        ESP_LOGW(TAG,
                 "NETWORK_DEGRADED ret=%s; entering offline-capable mode",
                 esp_err_to_name(offline_ret));
        c5_startup_stage_transition(C5_STARTUP_OFFLINE_MODE,
                                    network_ret == ESP_OK ? "gateway_recovery_pending" :
                                                        "wifi_or_gateway_unavailable",
                                    offline_ret);
    }
    ...
    ESP_LOGI(TAG, "C52_BOOT_STAGE stage=before_startup_task_exit");
