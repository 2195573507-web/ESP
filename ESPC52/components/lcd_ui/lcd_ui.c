...
/* The bundled LVGL font is Latin-only. These labels are the visible form of
 * the Chinese product states "yi huan xing" and "qing shuo". */
static const char *lcd_ui_voice_prompt(lcd_voice_state_t state)
{
    switch (state) {
    case LCD_VOICE_WAKE: return "WAKE CONFIRMED";
    case LCD_VOICE_RECORDING: return "SPEAK NOW";
    case LCD_VOICE_WAITING: return "PROCESSING";
    case LCD_VOICE_PLAYING: return "RESPONDING";
    case LCD_VOICE_ERROR: return "VOICE UNAVAILABLE";
    case LCD_VOICE_IDLE:
    default: return "";
    }
}
...
static esp_err_t lcd_ui_create(lcd_ui_context_t *ui)
{
    ...
    ui->voice_state_label = lcd_ui_label(ui->voice_overlay,
                                         ui->voice_state_text,
                                         24,
                                         204,
                                         192,
                                         lv_color_hex(LCD_UI_TITLE_COLOR));
    if (ui->voice_state_label == NULL) goto no_mem;
    lv_obj_set_style_text_align(ui->voice_state_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
                  ...
                  bool boot_complete)
{
    ...
    if (ui->voice_overlay != NULL && ui->dashboard != NULL) {
        ...
        (void)snprintf(ui->voice_state_text,
                       sizeof(ui->voice_state_text),
                       "%s",
                       lcd_ui_voice_prompt(snapshot->voice_state));
        lcd_ui_set_text(ui->voice_state_label, ui->voice_state_text);
        if (active) {
