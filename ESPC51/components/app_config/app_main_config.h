 ...
 * 4. MAIN_ENABLE_CSI_SERVICE=1 时在 WiFi 稳定后启动 CSI raw 接收和本地摘要日志。
 */
...
/* CSI 运行总开关：ESPC51 默认开启；置 0 时不配置 WiFi CSI、不启动 CSI 任务。 */
#define MAIN_ENABLE_CSI_SERVICE 1
#endif

#ifndef CSI_SERVICE_REPORT_INTERVAL_MS
/* CSI 摘要串口日志周期，单位 ms；只输出 occupancy/motion_score 等轻量结果。 */
#define CSI_SERVICE_REPORT_INTERVAL_MS 100U
#endif
...
#error "CSI_SERVICE_REPORT_INTERVAL_MS must be at least 100"
#endif
