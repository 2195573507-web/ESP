...
static void csi_service_rx_cb(void *ctx, wifi_csi_info_t *data)
{
    ...
    (void)csi_service_store_pending_sample(data);
}
...
esp_err_t csi_service_start(void)
{
    ...
    s_csi_paused = false;
    ESP_LOGI(TAG,
             "CSI service started: scheduler-driven calibration first, feature-only output interval_ms=%u log=%d http=%d feature_version=%s",
