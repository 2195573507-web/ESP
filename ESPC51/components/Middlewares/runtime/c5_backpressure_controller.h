 ...
 *
 * 周期性 C5 工作按 task type 分类。本模块统一决定本轮是否运行、下一次间隔如何放大；
 * gateway_link 和 voice_chain 只作为只读输入。它不改变业务 payload，也不让 C5
 * 直接访问 Server，所有上云路径仍只到 ESPS3 local gateway。
 */
...
/** @brief 启动 C5 统一调度任务；可重复调用。 */
esp_err_t c5_scheduler_start(void);

/** @brief 执行一次调度扫描；测试或手动 tick 可直接调用。 */
