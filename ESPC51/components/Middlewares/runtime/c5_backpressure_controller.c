 ...
 * @brief C5 终端统一回压控制器和自适应调度器。
 *
 * 本文件属于 ESP32-C5 终端调度层（当前 ESPC51 侧实现）。它把 CSI feature、本地 BME
 * 上报、heartbeat/status/command poll 放进同一个轻量调度循环，按语音独占、S3
 * local gateway 连接状态、待运行任务比例和 CPU 空闲估算决定跳过或放慢普通任务。
 * 本模块只调度 C5 本地工作，不构造 Server API，也不绕过 ESPS3。
 */
...
static c5_scheduled_task_t s_scheduler_tasks[] = {
    {C5_TASK_TYPE_CSI_PROCESS, 0},
    {C5_TASK_TYPE_CSI_REPORT, 0},
    {C5_TASK_TYPE_BME_SENSOR, 0},
    {C5_TASK_TYPE_SYSTEM_HEARTBEAT, 0},
    {C5_TASK_TYPE_SYSTEM_STATUS, 0},
    {C5_TASK_TYPE_SYSTEM_COMMAND_POLL, 0},
};
...
{
    size_t due_count = 0;
    size_t task_count = sizeof(s_scheduler_tasks) / sizeof(s_scheduler_tasks[0]);

    for (size_t i = 0; i < task_count; ++i) {
        if (s_scheduler_tasks[i].next_run_ms == 0U ||
            now_ms >= s_scheduler_tasks[i].next_run_ms) {
            ++due_count;
        }
    }

    uint8_t queue_load = task_count > 0U ? (uint8_t)((due_count * 100U) / task_count) : 0U;
    c5_backpressure_set_queue_load(queue_load);
}

static esp_err_t c5_scheduler_run_task(c5_task_type_t task_type)
{
    if (!c5_should_run(task_type)) {
        return ESP_ERR_INVALID_STATE;
    }

    switch (task_type) {
    case C5_TASK_TYPE_CSI_PROCESS:
        return csi_service_process_tick();
    case C5_TASK_TYPE_CSI_REPORT:
        return csi_service_report_tick();
    case C5_TASK_TYPE_BME_SENSOR:
        return bme_sensor_service_tick();
    case C5_TASK_TYPE_SYSTEM_HEARTBEAT:
        return system_service_tick_heartbeat();
    case C5_TASK_TYPE_SYSTEM_STATUS:
        return system_service_tick_status();
    case C5_TASK_TYPE_SYSTEM_COMMAND_POLL:
        return system_service_tick_command_poll();
    default:
...
void c5_scheduler_tick(void)
{
    ...
    for (size_t i = 0; i < task_count; ++i) {
        c5_scheduled_task_t *task = &s_scheduler_tasks[i];
        if (task->next_run_ms != 0U && now_ms < task->next_run_ms) {
            continue;
        }

        /* 即使本轮被回压跳过，也重新计算下一次时间，避免任务在队列里持续占满 load。 */
        esp_err_t ret = c5_scheduler_run_task(task->task_type);
        if (ret != ESP_OK &&
            ret != ESP_ERR_INVALID_STATE &&
            ret != ESP_ERR_NOT_FOUND) {
            ESP_LOGW(TAG,
                     "task tick failed task=%s ret=%s",
...
static uint32_t c5_scheduler_next_delay_ms(void)
{
    ...
    for (size_t i = 0; i < task_count; ++i) {
        if (s_scheduler_tasks[i].next_run_ms == 0U ||
            s_scheduler_tasks[i].next_run_ms <= now_ms) {
            return C5_SCHEDULER_MIN_SLEEP_MS;
        }
        uint64_t delay_ms = s_scheduler_tasks[i].next_run_ms - now_ms;
...
static void c5_scheduler_task(void *arg)
{
    (void)arg;
    ESP_LOGI(TAG,
             "C5 scheduler started stack=%u priority=%u",
...
esp_err_t c5_scheduler_start(void)
{
    ...
    BaseType_t created = xTaskCreate(c5_scheduler_task,
                                     "c5_scheduler",
    ...
    if (created != pdPASS) {
        ...
        ESP_LOGE(TAG, "create C5 scheduler task failed");
