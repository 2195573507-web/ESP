...
#ifndef VOICE_CHAIN_NETWORK_RETRY_MS
#define VOICE_CHAIN_NETWORK_RETRY_MS 500U
#endif

#ifndef VOICE_CHAIN_NETWORK_RETRY_LOG_INTERVAL
#define VOICE_CHAIN_NETWORK_RETRY_LOG_INTERVAL 20U
#endif
...
{
    portENTER_CRITICAL(&s_wake_prepare_lock);
    s_wake_prepare_in_progress = in_progress;
    portEXIT_CRITICAL(&s_wake_prepare_lock);
}

static bool voice_chain_wake_prepare_is_in_progress(void)
{
    bool in_progress;
    portENTER_CRITICAL(&s_wake_prepare_lock);
    in_progress = s_wake_prepare_in_progress;
    portEXIT_CRITICAL(&s_wake_prepare_lock);
    return in_progress;
}
...
esp_err_t voice_chain_request_local_wake(void)
{
    ...
    const voice_chain_item_t item = {
        .type = VOICE_CHAIN_EVENT_LCD_WAKE_REQUEST,
...
static void voice_chain_mark_mic_start_pending(const char *reason, esp_err_t ret)
{
    ...
    if (s_voice.state != VOICE_IDLE) {
        voice_chain_set_state(VOICE_IDLE);
    }
    if (s_voice.mic_start_attempts == 1U ||
        (s_voice.mic_start_attempts % VOICE_CHAIN_NETWORK_RETRY_LOG_INTERVAL) == 0U) {
        ESP_LOGI(TAG,
                 "VOICE_MIC_START_WAITING reason=%s attempt=%lu ret=%s wifi_stable=%d gateway=%s",
                 reason != NULL ? reason : "none",
...
static esp_err_t voice_chain_try_start_mic_vad(const char *reason)
{
    ...
    if (ret != ESP_OK) {
        ...
        ESP_LOGE(TAG,
                 "VOICE_MIC_START_FATAL reason=%s ret=%s",
                 reason != NULL ? reason : "none",
                 esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        voice_chain_set_state(VOICE_ERROR);
        ESP_LOGE(TAG,
                 "VOICE_MIC_START_FATAL reason=%s stage=wait_running ret=%s",
                 reason != NULL ? reason : "none",
                 esp_err_to_name(ret));
        return ret;
    }
    ...
    if (!local_wake_word_is_recording_window_open()) {
        ...
        if (!gateway_link_can_start_voice_turn()) {
            ...
            (void)voice_chain_queue_terminal_error(ESP_ERR_INVALID_STATE,
                                                    "gateway offline after voice resource quiesce");
            return ESP_ERR_INVALID_STATE;
        }
...
{
    (void)c5_task_delete_safe(&s_voice.task,
                               &s_voice_task_lock,
                               TAG,
                               "voice_chain",
                               "start_failure");
    if (s_voice_task_stack != NULL) {
        c5_mem_free(s_voice_task_stack, "voice_chain_stack");
        s_voice_task_stack = NULL;
    }
...
static void voice_chain_task(void *arg)
{
    ...
    while (1) {
        ...
        if (!voice_chain_take_terminal_event(&item) &&
            ...
            (void)ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(VOICE_CHAIN_NETWORK_RETRY_MS));
...
esp_err_t voice_chain_start(void)
{
    ...
    TaskHandle_t task = xTaskCreateStatic(voice_chain_task,
                                          ...
                                          s_voice_task_stack,
                                          &s_voice_task_storage);
    if (task == NULL) {
        ret = ESP_ERR_NO_MEM;
        voice_chain_log_start_stage("voice_chain_task_create", "after", ret);
        voice_chain_log_start_failure("xTaskCreateStatic(voice_chain)", ret);
        voice_chain_cleanup_start_failure();
        c5_mem_log("task_create_after_voice_chain_failed");
        return ret;
    }
    c5_task_handle_publish(&s_voice.task, &s_voice_task_lock, task);
    ...
    if (ret == MIC_ADC_ERR_NOT_READY) {
        ESP_LOGI(TAG, "VOICE_CHAIN_READY_WAITING_FOR_WIFI");
        return ESP_OK;
    }
    if (ret != ESP_OK) {
